...
os.chdir('user/' + getIpAdress() + '/output/')
...
def getSeqLengthMax(fileName = "user/" + getIpAdress() + "/input/upload_gene.fasta"):
    len_seq_max = 0
...
def update_output(value):
    if os.path.exists("user/" + getIpAdress() + "/input/upload_gene.fasta"):
        ref_genes_len = getSeqLengthMax("user/" + getIpAdress() +"/input/upload_gene.fasta")
        if value == None:
...
def parse_fasta_contents(contents, filename, date):
    ...
    try:
        if 'fasta' in filename:
            ...
            with open("user/" + getIpAdress() + "/input/upload_gene.fasta", "w") as f:
                f.write(seq_upload)
...
def update_output(n_clicks, bootstrap_threshold, rf_threshold, window_size, step_size, data_names):
    if n_clicks is None:
        ...
        reference_gene_file = '../input/upload_gene.fasta'
        pipeline_user.createPhylogeneticTree(reference_gene_file, window_size, step_size, bootstrap_threshold, rf_threshold, data_names)
